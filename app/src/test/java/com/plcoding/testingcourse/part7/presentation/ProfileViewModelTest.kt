...
internal class ProfileViewModelTest {
    ...
    @Test
    fun `Test loading profile success, state is properly updated`() = runTest {
    ...
    @Test
    fun `Test loading profile error, state is properly updated`() = runTest {
        repository.errorToReturn = Exception("Test error")
        viewModel.loadProfile()

        advanceUntilIdle()

        assertThat(viewModel.state.value.profile).isNull()
        assertThat(viewModel.state.value.errorMessage).isEqualTo("Test error")
        assertThat(viewModel.state.value.isLoading).isFalse()
    }
}